...
esp_console_cmd_t commands[CMD_COUNT] = {
    {
        ...
        .command = "set",
        .hint = USAGE_SET,
        .help = "Set a variety of variables that affect various components of the application. Usage: set <variable> <value>   <variable>   SCRAMBLE_WORDS: When generating random SSIDs as part of beacon, probe or fuzz, instead of creating SSIDs as a sequence of random words create them as a sequence of random letters.  SSID_LEN_MIN: Minimum length of a generated SSID   SSID_LEN_MAX: Maximum length of a generated SSID   MAC_RAND: Whether to change the device's MAC after each packet (default: ON)   DEFAULT_SSID_COUNT: Number of SSIDs to generate if not specified   CHANNEL: Wireless channel   MAC: ASP32C6's MAC address   HOP_MILLIS: Milliseconds to stay on a channel before hopping to the next (0: Hopping disabled)   ATTACK_PKTS: Number of times to repeat an attack packet when launching an attack (0: Don't stop attacks based on packet count)   ATTACK_MILLIS: Milliseconds to run an attack for when it is launched (0: Don't stop attacks based on duration)   NB: If ATTACK_PKTS and ATTACK_MILLIS are both 0 attacks will not end automatically but will continue until terminated.",
        .func = cmd_set
    }, {
        .command = "get",
        .hint = USAGE_GET,
        .help = "Get a variety of variables that affect various components of the application. Usage: get <variable>   <variable>   SCRAMBLE_WORDS: When generating random SSIDs as part of beacon, probe or fuzz, instead of creating SSIDs as a sequence of random words create them as a sequence of random letters.  SSID_LEN_MIN: Minimum length of a generated SSID   SSID_LEN_MAX: Maximum length of a generated SSID   MAC_RAND: Whether to change the device's MAC after each packet (default: ON)   CHANNEL: Wireless channel   MAC: ASP32C6's MAC address   HOP_MILLIS: Milliseconds to stay on a channel before hopping to the next (0: Hopping disabled)   ATTACK_PKTS: Number of times to repeat an attack packet when launching an attack (0: Don't stop attacks based on packet count)   ATTACK_MILLIS: Milliseconds to run an attack for when it is launched (0: Don't stop attacks based on duration)   NB: If ATTACK_PKTS and ATTACK_MILLIS are both 0 attacks will not end automatically but will continue until terminated.",
        .func = cmd_get
    }, {
        .command = "view",
        .hint = USAGE_VIEW,
